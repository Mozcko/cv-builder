 ...
 *   Summary text...
 */
function parseHeader(content, warnings) {
 ...
 *   - Bullet 2
 */
function parseExperience(content, warnings) {
  const experiences = [];
 ...
 *   - **Category:** Items
 */
function parseCategoryList(content) {
  const items = [];
  const lines = content.split('\n');
  for (const line of lines) {
 ...
 */
function parseEducation(content, warnings) {
  const educations = [];
 ...
 *   - Bullet 1
 */
function parseProjects(content, _warnings) {
  const projects = [];
  // Split by ### headings
 ...
 */
function parseSkillsComposite(content, _warnings) {
  const result = {
    skills: [],
    certifications: [],
    languages: '',
 ...
 *   Description text
 */
function parseCustomSection(title, content) {
 ...
 */
export function parseMarkdownToCV(markdown, lang = 'en') {
  resetIdCounter();
  const warnings = [];
